...
template<typename Kernel>
typename Kernel::Point_3 support_point(const CGAL::Kd_tree_rectangle<typename Kernel::FT, CGAL::Dimension_tag<3>>& rect, typename Kernel::Vector_3 direction){
    typename Kernel::Point_3 point;
    typename Kernel::FT max_scalar_product;
...
template<typename Kernel>
typename Kernel::Point_3 get_minkowski_diff_point(const CGAL::Kd_tree_rectangle<typename Kernel::FT, CGAL::Dimension_tag<3>>& rect, const typename Kernel::Tetrahedron_3& tet, typename Kernel::Vector_3 direction){
    return CGAL::Origin() + (support_point<Kernel>(rect, direction) - support_point<Kernel>(tet, -direction));
}
...
typedef typename Kernel::Tetrahedron_3 Tetrahedron_3;
    switch (simplex.size()) {
        ...
        case 2:
        {
            return Segment_3(simplex[0], simplex[1]).has_on(CGAL::Origin());
        }
        case 3:
        {
            return Triangle_3(simplex[0], simplex[1], simplex[2]).has_on(CGAL::Origin());
        }
        case 4:
        {
            CGAL::Bounded_side side = Tetrahedron_3(simplex[0], simplex[1], simplex[2], simplex[3]).bounded_side(CGAL::Origin());
